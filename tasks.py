...
def get_excluded_movies_from_radarr(radarr_url, radarr_api_key):
    headers = {"X-Api-Key": radarr_api_key}
    response = requests.get(f"{radarr_url}/api/v3/exclusions", headers=headers)
    if response.status_code == 200:
        exclusions = response.json()
        excluded_titles = [movie['movieTitle'] for movie in exclusions]
        return excluded_titles
    else:
        logger.error(f"Error fetching excluded movies list: {response.status_code} {response.reason}")
        return []
...
def run_sync_movies():
    ...
    # Obtener la lista de películas excluidas desde Radarr
    try:
        excluded_movies = get_excluded_movies_from_radarr(radarr_url, radarr_api_key)
        logger.info(f"Películas excluidas obtenidas: {excluded_movies}")
    except Exception as e:
        logger.error(f"Error fetching excluded movies list: {e}")
        return

    # Filtrar las películas según los criterios especificados
    filtered_movies = filter_items(movies_list, movies_min_year, movies_max_year, movies_min_rating, tmdb_api_key, 'movie')

    # Excluir las películas que están en la lista de exclusión
